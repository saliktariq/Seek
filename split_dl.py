...
"""
links_code += (
    extract_block(dl_code, r"^(def is_youtube_url.*?)(?=\ndef is_single_video_url)")
    + "\n"
)
links_code += (
    extract_block(
        dl_code, r"^(def is_single_video_url.*?)(?=\ndef normalize_youtube_url)"
    )
    + "\n"
)
links_code += (
    extract_block(
        dl_code, r"^(def normalize_youtube_url.*?)(?=\ndef parse_url_entries)"
    )
    + "\n"
)
links_code += (
    extract_block(dl_code, r"^(def parse_url_entries.*?)(?=\ndef parse_url_list)")
    + "\n"
)
...
system_code += (
    extract_block(
        dl_code, r"^(_JAVASCRIPT_RUNTIMES =.*?_RUNTIME_DISPLAY_NAMES =.*?})\n\n"
    )
    + "\n"
)
system_code += (
    extract_block(dl_code, r"^(def check_dependencies.*?)(?=\ndef _find_executable)")
    + "\n"
)
system_code += (
    extract_block(
        dl_code, r"^(def _find_executable.*?)(?=\ndef detect_javascript_runtimes)"
    )
    + "\n"
)
system_code += (
    extract_block(
        dl_code, r"^(def detect_javascript_runtimes.*?)(?=\nclass DownloadEvent)"
    )
    + "\n"
)
...
journal_code += (
    extract_block(
        dl_code, r"^(def _is_nonempty_file.*?)(?=\ndef _is_complete_video_folder)"
    )
    + "\n"
)
journal_code += (
    extract_block(
        dl_code, r"^(def _is_complete_video_folder.*?)(?=\ndef _infer_video_stage)"
    )
    + "\n"
)
journal_code += (
    extract_block(dl_code, r"^(def _infer_video_stage.*?)(?=\nclass CompletionIndex)")
    + "\n"
)
journal_code += extract_block(
    dl_code, r"^(class CompletionIndex.*?(?=\ndef is_youtube_url))"
)
